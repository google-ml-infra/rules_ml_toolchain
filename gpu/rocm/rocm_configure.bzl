...
# Define enable_cuda and enable_sycl locally to avoid circular dependencies
# with cuda_configure.bzl and sycl_configure.bzl
...
def _tpl_path(repository_ctx, labelname):
    ...
    labelname formats:
      - "rocm:build_defs.bzl" -> //gpu/rocm:build_defs.bzl.tpl
      - "rocm:BUILD" -> //gpu/rocm:BUILD.tpl
      - "crosstool:BUILD.rocm" -> //gpu/rocm/crosstool:BUILD.rocm.tpl
      - "crosstool:clang/bin/crosstool_wrapper_driver_rocm" ->
            //gpu/rocm/crosstool/clang/bin:crosstool_wrapper_driver_rocm.tpl
    """
    if labelname.startswith("rocm:"):
        # rocm:xxx -> //gpu/rocm:xxx.tpl
        return repository_ctx.path(Label("//gpu/rocm:%s.tpl" % labelname[5:]))
    elif labelname.startswith("crosstool:"):
        # crosstool:xxx -> need to handle paths with subdirectories
        path = labelname[10:]  # e.g., "BUILD.rocm" or "clang/bin/crosstool_wrapper_driver_rocm"
        if "/" in path:
            # Has subdirectory: clang/bin/crosstool_wrapper_driver_rocm -> //gpu/rocm/crosstool/clang/bin:crosstool_wrapper_driver_rocm.tpl
            last_slash = path.rfind("/")
            dir_part = path[:last_slash]
            file_part = path[last_slash + 1:]
            return repository_ctx.path(Label("//gpu/rocm/crosstool/%s:%s.tpl" % (dir_part, file_part)))
        else:
            # No subdirectory: BUILD.rocm -> //gpu/rocm/crosstool:BUILD.rocm.tpl
            return repository_ctx.path(Label("//gpu/rocm/crosstool:%s.tpl" % path))
    else:
