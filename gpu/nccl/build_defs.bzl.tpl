...
def cuda_rdc_library(name, hdrs = None, copts = None, linkstatic = True, defines = [], **kwargs):
    ...
    _device_link(
        ...
        nvlink_args = select({
            "@rules_ml_toolchain//common:is_linux_x86_64": ["--cpu-arch=X86_64"],
            "@rules_ml_toolchain//common:is_linux_ppc64le": ["--cpu-arch=PPC64LE"],
            "//conditions:default": [],
        }),
    )
    ...
    _merge_archive(
        ...
        srcs = [pruned, dlink],
        ar = select({
            "@bazel_tools//src/conditions:linux_x86_64": "@llvm_linux_x86_64//:llvm-ar",
            "@bazel_tools//src/conditions:linux_aarch64": "@llvm_linux_aarch64//:llvm-ar"
        }),
        distro_libs = select({
