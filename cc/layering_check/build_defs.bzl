...
def _generate_crosstool_cppmap_impl(ctx):
...
generate_crosstool_cppmap = rule(
    implementation = _generate_crosstool_cppmap_impl,
    attrs = {
...
def cc_toolchain_with_crosstool_cppmap(name, all_files, **kwargs):
    generate_crosstool_cppmap(name = "{}_crosstool_cppmap".format(name), targets = [all_files], out_file = "{}_crosstool.cppmap".format(name))
    cc_toolchain(
        name = name,
        all_files = all_files,
        module_map = "{}_crosstool.cppmap".format(name),
        **kwargs
    )
