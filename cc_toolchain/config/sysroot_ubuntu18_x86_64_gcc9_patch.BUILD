...
cc_toolchain_import(
    ...
    hdrs = glob([
        ...
        "usr/include/c++/{gcc_version}/experimental/**".format(gcc_version = HEADERS_VERSION),
    ]),
    includes = [
        "usr/include/c++/{gcc_version}".format(gcc_version = HEADERS_VERSION),
        "usr/include/x86_64-linux-gnu/c++/{gcc_version}".format(gcc_version = HEADERS_VERSION),
        "usr/include/c++/{gcc_version}/backward".format(gcc_version = HEADERS_VERSION),
        "usr/include/c++/{gcc_version}/experimental".format(gcc_version = HEADERS_VERSION),
    ],
    visibility = ["//visibility:public"],
)
